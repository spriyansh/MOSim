...
# To format the data into scMOSim-ready format, we pass the seurat
...
`scMOSim` can simulate scRNA, scATAC and TF count matrices without providing
...
sim <- scMOSim(omic_list, cell_types, TF = TRUE)
```
...
If you run the scMOSim simulation including TF, you can also extract the  
